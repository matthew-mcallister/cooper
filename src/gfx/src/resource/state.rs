...
use device::{BufferAlloc, BufferHeap, Image, ImageDef, ImageHeap, fmt_named};
...
impl ResourceStateTable {
    ...
    fn get_or_init_image(&mut self, image: &Arc<ImageDef>) ->
        &mut ResourceInfo<Image>
    {
    ...
    fn get_or_init_buffer(&mut self, buffer: &Arc<BufferDef>) ->
        &mut ResourceInfo<BufferAlloc>
    {
        let buffer = ByPtr::by_ptr(buffer);
        self.buffers.raw_entry_mut().from_key(&buffer)
            .or_insert_with(|| (ByPtr::clone(buffer), ResourceInfo {
                resource: None,
                batch: u64::MAX,
            })).1
    }
    ...
    crate fn alloc_buffer(
        &mut self,
        def: &Arc<BufferDef>,
        heap: &Arc<BufferHeap>,
    ) -> &mut Arc<BufferAlloc> {
        let alloc = Arc::new(heap.alloc(
            def.binding, def.lifetime, def.mapping, def.size));
    ...
    crate fn make_buffer_available(&mut self, def: &Arc<BufferDef>) {
    ...
    crate fn get_buffer(&self, buffer: &Arc<BufferDef>) ->
