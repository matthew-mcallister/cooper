...
impl SimdOps<3> for Vector3 {
    ...
    #[inline(always)]
    fn sum(self) -> f32 {
        self.0.extract(0) + self.0.extract(1) + self.0.extract(2)
    }
    ...
    ($Op:ident, $OpAssign:ident, $op:ident, $op_assign:ident) => {
            ...
            f32: Simd<N>,
        {
            ...
            fn $op_assign(&mut self, other: Vector<N>) {
                <Inner<N> as $OpAssign>::$op_assign(&mut self.0, other.0);
            }
        }
    };
}
...
mod tests {
    ...
    fn dot_and_cross() {
        ...
        assert_eq!(
            vec3(1.0, 2.0, -1.0).cross(vec3(2.0, -1.0, 1.0)),
            vec3(1.0, -3.0, -5.0)
        );
    ...
    fn inf_sup() {
        ...
        assert_eq!(
            vecs.iter().copied().inf_sup(),
            InfSupResult::InfSup(inf, sup)
        );

        assert_eq!([].iter().cloned().inf_sup::<Vector2>(), InfSupResult::Empty);
        let vecs = &[vec2(1.0, 1.0)];
        assert_eq!(
            vecs.iter().cloned().inf_sup(),
