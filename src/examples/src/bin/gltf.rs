...
use gfx::{
    MaterialDef, MeshInstance, PerspectiveParams, RenderLoop, RenderWorld, SceneView, WorldUniforms,
};
use math::{vec3, BBox3, MathItertools, Matrix3};
...
fn render_world(
    ...
    let rot = Matrix3::from([[-c, 0.0, -s], [0.0, -1.0, 0.0], [-s, 0.0, c]]);
        ...
        .map(|&idx| (idx as usize, &collection.nodes[idx as usize]))
    {
        match node.data {
            ...
            NodeData::Mesh(idx) => {
                ...
                for (prim, materials) in mesh.primitives.iter().zip(materials[idx].iter()) {
...
fn primitive_materials(rl: &mut RenderLoop, prim: &Primitive) -> MeshMaterials {
    ...
    macro_rules! define_material {
        ($rl:expr, $base:expr, $frag_stage:ident) => {{
            ...
            desc.stages[ShaderStage::Fragment] = Arc::clone(&$rl.specs().$frag_stage);
...
fn main() {
    unsafe {
        with_event_loop(main_with_proxy);
    }
}
