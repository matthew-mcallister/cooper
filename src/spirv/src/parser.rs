...
crate struct ShaderParser<'data> {
    ...
    crate entry_points: Vec<node::EntryPoint>,
    crate data: &'data [u32],
}
...
impl<'data> ShaderParser<'data> {
    ...
    fn parse_meta(&mut self) -> Result<()> {
        for inst in self.insts() {
            let mut inst = inst?;
            match inst.op {
                Some(Op::Source) => self.source = inst.parse()?,
                Some(Op::EntryPoint) => self.entry_points.push(inst.parse()?),
                _ => {},
            }
        }
        Ok(())
...
mod tests {
    ...
    fn example_test() {
        ...
        assert_eq!(parser.source.language, SourceLanguage::GLSL);
        assert_eq!(parser.source.version, 450);
        assert_eq!(parser.source.file, Some(1));
        assert!(parser.source.source.is_some());
        ...
        assert_eq!(
            &parser.entry_points[0],
            &node::EntryPoint {
                execution_model: ExecutionModel::Vertex,
                function: 5,
                name: "main".to_owned(),
                interface: vec![14, 29, 104, 119, 133, 135],
            },
        );
    }
}
